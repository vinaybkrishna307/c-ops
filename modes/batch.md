...
1. **Clasificación estricta:** ¿Es hard blocker o nice-to-have?
   - Hard blocker = el JD lo exige explícitamente O es el dominio central del rol (ej: "We use Haskell", "multi-DC active-active architecture", "Jenkins required")
   - Nice-to-have = "preferred", "a plus", "familiarity with", "beneficial"
   - **Si hay ≥1 hard blocker que el candidato no puede cubrir con experiencia adyacente → refléjalo en el Score Global con los hard caps del rubric**
2. ¿Puede el candidato demostrar experiencia adyacente creíble? (ser honesto — "adjacent" no significa "nunca lo he tocado pero es parecido")
3. ¿Hay un proyecto en el portfolio que cubra este gap directamente?
4. Plan de mitigación concreto — o "No mitigation possible: genuine domain gap"
...
**Rubric — scoring rules (MANDATORY, read before scoring):**

**Match con CV (0–5):**
- 5.0 → ≥85% of JD requirements map to explicit lines in cv.md with matching terminology
- 4.0 → 70–84% match; gaps are nice-to-haves or easily bridged
- 3.0 → 50–69% match; 1–2 hard requirements missing but adjacent experience exists
- 2.0 → 30–49% match; multiple hard requirements missing
- 1.0 → <30% match; fundamentally different domain or tech stack

**HARD CAPS on Match con CV — apply before scoring:**
- Any single missing hard requirement (language, platform, or domain that is CORE to the role, not optional) → cap at 3.5 max
- 2+ missing hard requirements → cap at 2.5 max
- Role requires a language/stack the candidate has never used (e.g. Haskell, Go, Rust, .NET) and that language is central to the role → cap at 2.0 max
- Role is in a fundamentally different engineering domain (distributed DB internals, hardware, embedded, game engine, etc.) → cap at 2.5 max

**Examples of hard requirements vs nice-to-haves:**
- Hard: "Must have X years in Y", "We use Haskell", "Active-active multi-DC architecture experience", "Jenkins required"
- Nice-to-have: "Familiarity with Z preferred", "Azure a plus", "Experience with chaos engineering beneficial"

**Alineación North Star (0–5):**
- Does this role match the candidate's target: Senior DevOps / SRE / Platform / MLOps in Bengaluru or remote-India, 15–22 LPA?
- 5.0 → exact title match, Bengaluru/remote, comp in range
- 4.0 → close title, minor location/comp deviation
- 3.0 → adjacent title or unclear comp
- 2.0 → title mismatch or out-of-range comp
- 1.0 → wrong domain or well below comp target

**Global score calculation:**
- Weighted average: Match (40%) + North Star (25%) + Comp (20%) + Cultural (15%) − Red flags
- **Global score CANNOT exceed the Match con CV score by more than 0.5 points.** If Match is 2.5, Global max is 3.0.
- A score ≥ 4.0 means: "Strong apply — candidate has a realistic chance of passing technical screen without significant prep." Do NOT give 4.0+ if there are hard domain mismatches.
- A score of 4.9–5.0 is reserved for roles where ≥90% of requirements match cv.md exactly. This should be rare.

**Anti-hallucination check before finalizing score:**
Before writing the score, answer these three questions:
1. Can the candidate pass a 60-minute technical screen for this role using only what is in cv.md? If No → score ≤ 3.5
2. Are there missing hard requirements that cannot be answered with adjacent experience? If Yes → apply hard caps above
3. Is the global score ≥ 4.0? If Yes → re-read cv.md and the JD and verify at least 3 explicit bullet-level matches for the core requirements. If you cannot find them, lower the score.
...
### Paso 3.5 — Recalcular score (OBLIGATORIO)

**⛔ ESTE PASO ES OBLIGATORIO. El score del Paso 2 es una estimación del AI — debe ser verificado y corregido por el calculador antes de continuar.**

Ejecuta:
```bash
node score-calculator.mjs reports/{{REPORT_NUM}}-{company-slug}-{{DATE}}.md
```

El script:
- Lee el YAML del report (hard_stops, soft_gaps, score)
- Aplica los hard caps del rubric de forma determinista
- Sobreescribe el score en el report .md
- Imprime JSON con `final_score` y `final_decision`

**Usa el `final_score` del output JSON para todo lo que sigue** (PDF gate, tracker line, output JSON del Paso 6). No uses el score original del Paso 2.

Si `score-calculator.mjs` no existe en el proyecto, créalo copiando el contenido de `score-calculator.mjs` del proyecto (o descárgalo desde el repo). Si el script falla, usa el score del Paso 2 pero añade una nota `"score_calculator": "failed"` en el output JSON.

### Paso 4 — Generar PDF (configurable)

**Gate:** Read `config/profile.yml` → `auto_pdf_score_threshold`. If the key is absent, default to **`3.0`** (the original gate of Path A). This step ONLY runs when the **`final_score` from Paso 3.5** is **≥ the resolved threshold**. For everything below it, skip this entire step — the user can generate a tailored PDF on demand later via `/career-ops pdf {company-slug}` using the report from Paso 3 as input.
...
The generated PDF must remain structurally identical to cv.md.

Allowed:
- Reorder existing bullets
- Reorder existing projects
- Reorder existing skills
- Inject truthful JD keywords
...
Never rewrite or reformat the title using:
- archetype
- company domain
- JD language
- inferred seniority
...
### Skills Preservation Rule

Preserve every skill category name exactly as written in cv.md (e.g. "Cloud & Infra" must remain "Cloud & Infra" and NOT be renamed to "Cloud"; "Kubernetes" must remain "Kubernetes" and NOT be renamed to "Tools").

Preserve every technology from cv.md. Do not remove any technology.
...
Reordering technologies within their categories, or reordering the categories themselves, is allowed. Adding relevant technologies that can be easily acquired is allowed.

### Summary Rules

Preserve Professional Summary.

Only inject JD keywords that accurately describe existing experience.
...
- architect titles
- platform engineer titles
- responsibilities not present in cv.md
...
Do not rewrite, optimize, reformat, infer, enhance, modernize, or adapt them. Do NOT add any markdown formatting (like wrapping them in asterisks `**` or other tags) to these fields in the output markdown. They must match cv.md exactly:
...
Only the following may be tailored:
- Professional Summary (up to 15% drift, no new claims)
- Bullet ordering
- Project ordering
- Skill ordering
- Keyword injection into existing content (without deleting existing technologies)

Validation Failure Conditions

The PDF generation must fail and regenerate if:

- Resume headline differs from cv.md
- Headline contains formatting like asterisks `**`
- Any company name differs from cv.md
- Any job title differs from cv.md
- Any job title contains formatting like asterisks `**`
- Any project title differs from cv.md
- Any skill category is removed or renamed
- Any technology is removed

If a field exists in cv.md, preserve it exactly.
1. Lee `cv.md` + `i18n.ts`
...
6. Preserve Professional Summary.Only inject JD keywords that accurately describe existing experience.Maximum modification: 15%.
   Do not introduce:
   - founding-team claims
   - startup claims
   - product ownership
...
10. Write the tailored markdown content to `reports/cv-candidate-${company-slug}.md`.
11. Compile the HTML and PDF by running:
```bash
node generate-pdf.mjs \
  reports/cv-candidate-{company-slug}.md \
  reports/cv-candidate-{company-slug}-{{DATE}}.pdf \
...
- Header title exactly matches cv.md
- All skill categories from cv.md are present
- All technologies from cv.md are present
- No Core Competencies section exists
- No Certifications section exists unless present in cv.md
- All project bullets are present
- All experience bullets are present
